...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.convt3=nn.Sequential(nn.ConvTranspose2d(32,3,4,2,1),
                                  )
        self.convt32=nn.Sequential(nn.ConvTranspose2d(64,32,4,2,1),
                                   nn.BatchNorm2d(64,momentum=0.9),
                                   nn.LeakyReLU()
                                  )
        self.convt64=nn.Sequential(nn.ConvTranspose2d(128,64,4,2,1),
                                   nn.BatchNorm2d(128,momentum=0.9),
                                   nn.LeakyReLU()
                                  )
        self.convt128=nn.Sequential(nn.ConvTranspose2d(256,128,4,2,1),
                                    nn.BatchNorm2d(256,momentum=0.9),                                  
                                    nn.LeakyReLU()
                                  )
        self.convt256=nn.Sequential(nn.BatchNorm2d(256, momentum=0.9),
                                    nn.LeakyReLU(),
        ...
        self.noise_emb=nn.Sequential(nn.Linear(356,512*4*4),
                                     )
    ...
    def forward(self,noise,label):
        ...
        noise=self.convt32(noise)             #noise:[-1,32,64,64]
        noise=self.convt3(noise)             #noise:[-1,3,128,128]
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv3=nn.Sequential(nn.Conv2d(3,32,4,2,1),
                                  nn.BatchNorm2d(3,momentum=0.9),
                                  nn.LeakyReLU(),
                                  nn.Dropout(0.7),
                                  )
        self.conv32=nn.Sequential(nn.Conv2d(32,64,4,2,1),
                                  nn.BatchNorm2d(32,momentum=0.9),
                                  nn.LeakyReLU(),
                                  nn.Dropout(0.7),
                                  )
        self.conv64=nn.Sequential(nn.Conv2d(64,128,4,2,1),
                                  nn.BatchNorm2d(64,momentum=0.9),
                                  nn.LeakyReLU(),
                                  nn.Dropout(0.7),
                                  )
        self.conv128=nn.Sequential(nn.Conv2d(128,256,4,2,1),
                                   nn.BatchNorm2d(128,momentum=0.9),
                                   nn.LeakyReLU(),
                                   nn.Dropout(0.7),
                                   )
        self.conv256=nn.Sequential(nn.Conv2d(256,512,4,2,1),
                                  nn.BatchNorm2d(256,momentum=0.9),
                                  nn.LeakyReLU(),
                                  nn.Dropout(0.7),
                                  )
        self.conv768=nn.Sequential(nn.Conv2d(768,512,1,1,0),
    ...
    def forward(self,x,label):       
        ...
        x=self.conv256(x)                   #x:[-1,512,4,4]
        x=torch.cat([x,label],dim=1)         #x:[-1,768,4,4]
        x=self.conv768(x)                    #x:[-1,512,4,4]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', '-mn', type=str, default='acGAN_4')
