...
def load_test(directory, batch_size):
    ...
    test_feat = list()
    video_id_list = open(os.path.join(directory,"training_data/id.txt"), 'r').read().split('\n')[:-1]
    for video_id in video_id_list:
        test_feat.append(np.load(os.path.join(directory, 'training_data/feat', video_id + '.npy')))
...
def main(args):
    ...
    with open(os.path.join('train_result',args.model_no+'train_result.csv'), "w+") as f:
        print('id,value', file=f)
        for i in range(len(pred_list)):
            video_id = pred_list[i][0][0]
            sent = str()
            stop = 0
