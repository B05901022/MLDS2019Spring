...
class EncoderDecoder(nn.Module):
    ...
    def forward(self,
        ...
        encode  = self.encoder(self.src_embed(src), src_mask)
        output  = self.decoder(self.tgt_embed(tgt), encode, src_mask, tgt_mask)
        return output
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.layers = nn.ModuleList([copy.deepcopy(layer) for i in range(N)])
...
class EncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.sublayer     = nn.ModuleList([copy.deepcopy(SublayerConnection(size, dropout)) for i in range(2)])
        self.size         = size
    def forward(self, x, mask):
...
class DecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.feed_forward = feed_forward
        self.sublayer     = nn.ModuleList([copy.deepcopy(SublayerConnection(size, dropout)) for i in range(3)])
    def forward(self, x, memory, src_mask, tgt_mask):
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask))
        x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask))
        output = self.sublayer[2](x, self.feedforward)
        return output
